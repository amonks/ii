...
func promptContextFromRepo(workDir, globalConfigDir string) (PromptContent, error) {
	...
	testCommands, err := loadTestCommands(workDir)
	if err != nil {
		return PromptContent{}, err
	}
...
func loadTestCommands(workDir string) ([]string, error) {
	cfg, err := config.Load(workDir)
	if err != nil {
		return nil, err
	}
	return cfg.Job.TestCommands, nil
}
