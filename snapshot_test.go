...
func TestBatchSnapshotTotals(t *testing.T) {
	...
	recipe := []RecipeComponent{
		{Ingredient: NewLotDescriptor(cream), MassKg: 60},
		{Ingredient: NewLotDescriptor(sugar), MassKg: 40},
	}
...
func TestBatchSnapshotFormulationBreakdownMatchesRecipe(t *testing.T) {
	components := []RecipeComponent{
		{Ingredient: NewLotDescriptor(SpecFromComposition("Cream", PointComposition(0.36, 0.08, 0, 0))), MassKg: 60},
		{Ingredient: NewLotDescriptor(SpecFromComposition("Sugar", PointComposition(0, 0, 1, 0))), MassKg: 40},
	}
