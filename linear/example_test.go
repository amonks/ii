...
func TestWorkflow2_FormulationToRecipe(t *testing.T) {
	...
	target := linear.Composition{
		Fat:   linear.Range(0.15, 0.17),
		MSNF:  linear.Range(0.09, 0.11),
		Sugar: linear.Range(0.13, 0.15),
		Other: linear.Range(0, 0.01),
	}
...
func TestWithTighterConstraints(t *testing.T) {
	...
	solver, _ := linear.NewSolver(problem)
	...
	solver2, _ := linear.NewSolver(problem)
	...
	solver3, _ := linear.NewSolver(problem)
