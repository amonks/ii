...
func main() {
	...
	registry := map[string]func() (*linear.LabelScenarioResult, error){
		"ben":      linear.SolveBenAndJerryVanilla,
		"jenis":    linear.SolveJenisSweetCream,
		"haagen":   linear.SolveHaagenDazsVanilla,
		"brighams": linear.SolveBrighamsVanilla,
		"breyers":  linear.SolveBreyersVanilla,
		"talenti":  linear.SolveTalentiVanilla,
	}
	...
	results := make([]*linear.LabelScenarioResult, 0, len(selected))
	...
	if *htmlOutput != "" {
		html, err := linear.RenderLabelReport(results)
		if err != nil {
...
func printScenario(result *linear.LabelScenarioResult) {
	fmt.Printf("\n=== %s ===\n\n", result.Name)
