...
import (
	...
	linear "github.com/amonks/creamery/linear"
)
...
func main() {
	...
	pantry := linear.DetailedIngredientTable()

	keys := make([]string, 0, len(components))
	weights := make([]float64, 0, len(components))
	table := make(map[string]linear.DetailedIngredient, len(components))
	recipeComponents := make([]linear.RecipeComponent, 0, len(components))
	...
	for _, comp := range components {
		...
		recipeComponents = append(recipeComponents, linear.RecipeComponent{
			Ingredient: &entry,
			MassKg:     mass,
		})
	}

	recipe, err := linear.NewRecipe(recipeComponents, 0.0)
	if err != nil {
		log.Fatal(err)
	}

	opts := linear.MixOptions{
		ServeTempC: linear.DefaultServeTempC(),
		DrawTempC:  linear.DefaultDrawTempC(),
		ShearRate:  linear.DefaultShearRate(),
	}

	metrics := linear.BuildProperties(keys, weights, table, opts)
	...
	snapshot := linear.ProductionSettings{
	...
	servingSize, err := recipe.ServingSizeForVolume(linear.ServingPortionLiters(), opts)
	if err != nil {
		log.Fatal(err)
	}
...
func printRecipe(r *linear.Recipe) {
	fmt.Println("Recipe:")
...
func printFormulation(f linear.Formulation) {
...
func printNutrition(n linear.NutritionFacts) {
