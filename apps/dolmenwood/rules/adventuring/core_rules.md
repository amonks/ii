...
Most situations of peril and uncertainty are resolved using one of a handful of core game rules: a Skill Check, Ability Check, Saving Throw, or Attack Roll. In situations where none of these core rules obviously applies, the Referee may simply judge a chance of success and ask the player to roll. See [Chance Rolls](core_rules#chance_rolls).
...
**Listen Checks:** Rolled when a character presses their ear to a door to detect the presence of creatures on the other side (see [Doors](hazards_and_challenges#doors)).

**Search Checks:** Rolled when searching for secret doors or traps (see [Hidden Features](hazards_and_challenges#hidden_features)).

**Survival Checks:** Rolled when trying to find food in the wild (see [Finding Food in the Wild](hazards_and_challenges#finding_food_in_the_wild)).
...
An Ability Check represents a test of a character’s basic physical or mental aptitudes (one of their [Ability Scores](ability_scores)), rather than learned aptitudes due to their Class or Level.
...
Ability Checks apply in certain specific situations, when called for elsewhere in the rules (see the list under [Ability Scores](ability_scores)). The Referee may also optionally call for an Ability Check in other situations as desired.
...
An Attack Roll is made whenever one creature attempts to physically harm another, whether using fists, claws, melee weapons, or missile weapons. See [Combat](combat).

### Damage Rolls

The equipment list ([Armour and Weapons](armour_and_weapons)) notes the damage die rolled for each type of weapon. Likewise, the descriptions of creatures with natural attack forms (e.g. claws, fangs) note the damage die rolled.
...
**Recovering lost HP:** Lost Hit Points can be recovered by rest and healing. See [Rest and Healing](settlements#rest_and_healing).
