...
(() => {
  ...
  // streams.ts
  function initStreamViewers(running) {
    document.querySelectorAll(".stream-viewer").forEach((details) => {
      initOneStream(details, running);
    });
  }
  function initOneStream(details, running) {
    const url = details.dataset.streamUrl;
    if (!url)
      return;
    const pre = details.querySelector(".stream-output");
  ...
  function updateRunMetadata(state) {
  ...
  function updateJobsTable(state) {
    ...
    const existingStreams = /* @__PURE__ */ new Set();
    tbody.querySelectorAll(".stream-viewer").forEach((details) => {
    ...
    for (const job of state.jobs) {
      ...
      if (job.error) {
        ...
        let errorRow = document.getElementById(errorRowId);
        if (!errorRow) {
      ...
      for (const stream of jobStreams) {
        ...
        `;
          const newDetails = streamTr.querySelector(".stream-viewer");
