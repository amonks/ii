...
- CLI e2e suites live under `testdata` and are executed from `*_test.go`.
...
Agent CLI tests in `testdata/agent/*.txtar` exercise the `ii agent run` command against real LLM providers. These tests:
