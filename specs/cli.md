...
The version variables (`buildChangeID`, `buildCommitID`) are defined in `version.go` within the `main` package. Because they are in package `main`, the linker requires the `main.` prefix regardless of import path:
