...
## Defaults
- State: `~/.local/state/incrementum` (SQLite `state.db`)
...
- `DefaultDBPath() (string, error)`: returns the default SQLite state database path (`~/.local/state/incrementum/state.db`).
